...
<p align="center">
  基于 Cloudflare Workers + D1 的 API 驱动临时邮箱。<br>
  Webhook 原生。零 DNS。为机器而生，非浏览器。
</p>
...
MailDrop 不是网页版临时邮箱。它是一个**可编程邮箱后端**，服务对象是 AI Agent、CI/CD 流水线、自动化脚本。

- 🔌 **Webhook 原生**：一条 `POST /api/inbound` 即可将消息投递到邮箱。
- 🌐 **无需 DNS、无需 MX、无需 SMTP**：邮箱地址 (`xxx@mail.<你的域名>`) 只是 D1 里的一行字符串，不需要真实域名。
- ⚡ **零成本运行**：完全跑在 Cloudflare 免费额度内 —— 每天 10 万次请求，500 万次 D1 读取。
- 📦 **一条命令部署**：`wrangler deploy`。无构建步骤，无 Node 依赖。
- 🤖 **Agent 优先设计**：REST API + Bearer/auth-key 鉴权，JSON 输入输出 —— 直接接入任何 AI Agent 的工具循环。
...
| 你想要... | 方案 |
|-----------|------|
| 随手打开一个网页收验证码 | ❌ 随便找个临时邮箱网站 |
| 让 CI 脚本 POST 一条消息、用 API 轮询取回 | ✅ |
| 给 AI Agent 的工具函数接入一个收件能力 | ✅ |
| 拥有一个你自己控制的、零持续成本的邮箱后端 | ✅ |
...
worker 无 Node 依赖、无构建步骤。`wrangler deploy` 是唯一必需的部署命令。
...
worker 设计为完全运行在 Cloudflare 免费额度内：
...
鉴权开关存在但仅能防止未鉴权访问，不能阻止 token 泄露。worker URL 一旦公开，免费额度会被耗尽，部署对所有者本人也不再可用。

---

## 仓库规则

`lingion/maildrop` 是本项目的唯一主线仓库。镜像与 fork 不应被视为主要入口。所有开发在此处合并。
...
你可以使用、修改、再分发本作品，包括用于商业目的，前提是衍生作品同样以 GPL-3.0 授权并保留版权声明。不提供任何担保。

---

## 贡献

欢迎在 <https://github.com/lingion/maildrop> 提交 PR。提交贡献即视为同意以 GPL-3.0 授权你的贡献。
